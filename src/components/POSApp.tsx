...
const steps = [
  { id: 'inicio', label: 'Início', title: 'Painel do PDV', subtitle: 'Resumo rápido do dia com alertas e atalhos.', accent: '#FF4F00' },
  { id: 'venda', label: 'Venda', title: 'Registrar venda', subtitle: 'Selecionar itens, ajustar quantidades e confirmar a operação.', accent: '#4E18FF' },
  { id: 'estoque', label: 'Estoque', title: 'Monitorar estoque', subtitle: 'Visualizar criticidade e recomendar reposição.', accent: '#3D7700' },
  { id: 'entrega', label: 'Entrega', title: 'Acompanhar entrega', subtitle: 'Ver a próxima coleta e o status da rota.', accent: '#FECC14' },
] as const;

const products = [
  { name: 'Pão de Forma Tradicional', price: 'R$ 8,90', qty: 2 },
  { name: 'Bisnaguinha', price: 'R$ 6,40', qty: 1 },
  { name: 'Rap10 Integral', price: 'R$ 12,50', qty: 1 },
];

const stockItems = [
  { name: 'Pão de Forma Tradicional', stock: 2, status: 'critical', note: 'Reposição urgente em 2h' },
  { name: 'Bisnaguinha', stock: 15, status: 'warning', note: 'Cobertura para o restante do dia' },
  { name: 'Rap10 Integral', stock: 45, status: 'good', note: 'Estoque estável' },
] as const;

const deliveries = [
  { name: 'CD Vila Mariana', distance: '2,5 km', eta: '10 min', status: 'Disponível' },
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto flex flex-col items-center justify-center min-h-[calc(100vh-4rem)]">
      <div className="text-center mb-8 max-w-3xl">
        ...
        <h1 className="text-4xl font-bold tracking-tight text-black mb-4">Fluxo interativo do uso diário do PDV</h1>
        <p className="text-gray-500 text-lg">
          Demonstração navegável do app do cliente, com registro de vendas, acompanhamento de estoque e visualização de entregas.
        </p>
      </div>
      ...
      <div className="w-full max-w-5xl mb-6">
          ...
          {steps.map((step, index) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-bold transition-all border ${isActive ? 'bg-black text-white border-black shadow-lg' : 'bg-white text-gray-500 border-gray-200 hover:border-gray-300'}`}
              >
                {index + 1}. {step.label}
              </button>
            );
        ...
        <div className="bg-white rounded-[24px] border border-gray-200 shadow-sm p-4">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-gray-400">Etapa atual</p>
              <h2 className="text-2xl font-black text-black">{current.title}</h2>
        ...
        <div className="bg-[#F7F7F4] w-full h-full rounded-[38px] overflow-hidden flex flex-col relative">
          <div className="pt-12 pb-6 px-6 text-black rounded-b-3xl shadow-md relative z-10" style={{ background: `linear-gradient(135deg, ${current.accent}, #111111)` }}>
            <div className="flex justify-between items-start mb-6 gap-4">
              <div>
                ...
                <p className="text-xs font-medium text-white/70 mt-1">Protótipo interativo do fluxo PDV</p>
              </div>
            ...
            <div className="bg-white/15 backdrop-blur-md rounded-2xl p-4 flex items-center justify-between border border-white/10">
              <div>
                <p className="text-xs font-bold uppercase opacity-80 text-white">Próxima entrega</p>
          ...
          <div className="flex-1 overflow-y-auto p-6 pb-24 space-y-6">
              ...
              {steps.map((step, index) => {
                ...
                return (
                  <button
                    key={step.id}
                    onClick={() => setActiveStep(index)}
                    className={`py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all ${isActive ? 'bg-black text-white' : 'bg-white text-gray-400 border border-gray-200'}`}
                  >
                    {step.label}
                  </button>
                );
            ...
            <div key={current.id} className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
              {activeStep === 0 && <HomeScene />}
              {activeStep === 1 && <SaleScene />}
              {activeStep === 2 && <StockScene />}
              {activeStep === 3 && <DeliveryScene />}
            </div>
          </div>
          ...
          <div className="absolute bottom-0 inset-x-0 bg-white border-t border-gray-100 px-6 py-4 flex justify-between items-center pb-8">
            <NavItem icon={Package} label="Início" active={activeStep === 0} />
            <NavItem icon={ShoppingCart} label="Venda" active={activeStep === 1} />
            <NavItem icon={MapPin} label="Entregas" active={activeStep === 3} />
  ...
  return (
    <>
      <div className="grid grid-cols-2 gap-3">
        ...
        <MiniStat icon={Clock3} label="Próxima coleta" value="14:30" accent="#4E18FF" />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <h3 className="font-bold text-lg text-black">Acesso rápido</h3>
          </div>
          <Sparkles size={18} className="text-[#FF4F00]" />
        </div>

        <div className="grid grid-cols-3 gap-3">
          <QuickAction icon={ShoppingCart} label="Venda" tone="bg-black text-white" />
          <QuickAction icon={Package} label="Estoque" tone="bg-[#FFF2EA] text-[#FF4F00]" />
          <QuickAction icon={Truck} label="Entregas" tone="bg-[#FEF7DA] text-black" />
        </div>
      </div>
      ...
      <div className="bg-black text-white rounded-2xl p-4 shadow-xl">
        ...
        <div className="flex items-start gap-3 mt-3">
          ...
          <div>
            <p className="font-black">Produto crítico detectado</p>
            <p className="text-sm text-white/70 mt-1">Pão de Forma Tradicional abaixo do limite mínimo. Ação recomendada: registrar venda e solicitar reposição.</p>
          </div>
        </div>
      </div>
    </>
  ...
  return (
    <>
      <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
        ...
        <button className="w-full mt-4 bg-[#4E18FF] text-white font-bold py-4 rounded-2xl shadow-lg flex items-center justify-center gap-2">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <h3 className="font-bold text-lg text-black">Produtos críticos</h3>
          </div>
        ...
        <div className="space-y-3">
          {stockItems.map((item) => (
            <div key={item.name}><StockRow name={item.name} stock={item.stock} status={item.status} note={item.note} /></div>
          ))}
        </div>
      </div>

      <div className="bg-[#EDF7E9] rounded-2xl p-4 border border-[#d7ebcf]">
        <p className="text-xs font-bold uppercase tracking-wider text-[#3D7700]">Sugestão</p>
        <p className="font-bold text-black mt-1">Acionar reposição imediata para o item crítico e alertar a torre de controle.</p>
      </div>
    </>
  );
}
    ...
    <>
      <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <h3 className="font-bold text-lg text-black">Disponíveis para retirada</h3>
          </div>
      ...
      <div className="bg-[#FEF7DA] rounded-2xl p-4 border border-[#f2e4a2] flex items-start gap-3">
        ...
        <div>
          ...
          <p className="font-bold text-black mt-1">Próxima coleta confirmada para hoje às 14:30.</p>
        </div>
      </div>
    </>
  );
...
function QuickAction({ icon: Icon, label, tone }: { icon: any; label: string; tone: string }) {
  return (
    <div className={`rounded-2xl p-3 flex flex-col items-center gap-2 ${tone}`}>
      <Icon size={18} />
